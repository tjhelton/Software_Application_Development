//: ## Pet Problem
//: Imagine your friend is putting together a pet show and needs to keep track of how many animals of each kind will be in the show.
//: Your friend also needs to know how many total animals, and how many mammals will be present.
//:
//: Also imagine that all you have with you is a laptop with Xcode on it. Maybe you can use a playground to help your friend.
//:
//: Using what you know so far, you could keep track of the count for each animal using a comment and a number like this:
// Number Of Dogs
6

// Number Of Cats
5

// Number Of Turtles
2

// Number of Hamsters
1
//: Then you could copy those numbers onto a new lines of code and let the playground do some math for you, like this:
// Total Number Of Animals
6 + 5 + 2 + 1

// Total Number Of Mammals
6 + 5 + 1
//: - experiment: As the week goes on, you get more information about what pets will be in the show.\
//:Update the code above for the changes.
//:
//: For example, if another person is bringing a dog, keep track of the change by updating the `Number Of Dogs` from `5` to `6` and change the `5` to a `6` everywhere else it appears (`Total Number Of Animals` and `Total Number Of Mammals`).
//:
//: If someone is bringing a new kind of animal, use the same pattern as the other animal counts. Add a new comment (`// Number Of Hamsters` for instance) on one line with the correct number on the next line. Don’t forget to update the totals.
//:
//: - Another person is bringing a dog.
//: - One of the turtles got sick and won’t come.
//: - Another person is bringing a cat.
//: - Someone is bringing a hamster (a mammal).
//:
//: Once you’re done with the experiment, move on!
//:
//:[Previous](@previous)  |  page 2 of 14  |  [Next: Pet Problem 2](@next)
